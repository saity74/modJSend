...
class sendHelper{
    ...
    public static $client_greeting;

    public static $manager_subject;
    public static $manager_body;
    public static $manager_greeting;
        ...
        if ($position)
        {
            ...
            foreach($modules as $module)
            {
                if ($module->module == 'mod_jsend')
                {
                    ...
                    self::$client_greeting = $jsend_params->get('client_greeting');
                    
                    self::$manager_subject = $jsend_params->get('manager_subject');
                    self::$manager_body = $jsend_params->get('manager_body');
                    self::$manager_greeting = $jsend_params->get('manager_greeting');
            ...
            {
                self::$list_val = $jinput->getString('list', '');
            }
            
            if ($showemail = $jsend_params->get('showemail', 0) > 0)
            {
                self::$email = $jinput->getString('email', '');
            ...
            {
                self::$firm = $jinput->getString('firm', '');
            ...
            {
                self::$name = $jinput->getString('name', '');
            ...
            $app->redirect('/', JText::_('MOD_JSEND_PARAMS_ERROR'));
        }
    }
    ...
    public static function send()
    {
        ...
        if ($sent) $app->redirect('/', JText::_('MOD_JSEND_MAILSEND'));
    }
}
